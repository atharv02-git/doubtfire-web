...
//   NewUnitService,
//   NewUnitRoleService,
// } from 'src/app/common/services'; // Import necessary services
...
export class UnitStaffEditorComponent {
  ...
  selectedStaff: any = null; // This is the staff member selected by the user to be added
  ...
  ngOnInit() {
    ...
    this.unit.staffCache.values.subscribe((staff: any[]) => (this.unitStaff = staff));
  }

    // Method to display the staff name in the autocomplete dropdown
  displayStaffName(staff: any): string {
    return staff && staff.user ? staff.user.name : '';
  }

  // Method to change the role of a staff member (e.g., Tutor or Convenor)
  changeRole(unitRole: any, roleId: number) {
    ...
    this.unitRoleService.update(unitRole).subscribe({
      ...
      error: (response: any) => this.alertService.error(response, 6000),
    });
  }

  // Method to change the main convenor of the unit
  changeMainConvenor(staff: any) {
  ...
  addSelectedStaff() {
    ...
    this.unit.staff = this.unit.staff || []; // Ensure the unit staff array is initialized
    ...
    if (staff?.id) {
      ...
      this.unit.addStaff(staff).subscribe({
        ...
        error: (response: any) => this.alertService.error(response, 6000), // Show error alert
      });
    } else {
